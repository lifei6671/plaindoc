...
ALTER TABLE auth_captcha_challenges COMMENT = '认证验证码会话表：记录验证码题目、有效期和一次性消费状态';

ALTER TABLE auth_captcha_challenges
	MODIFY COLUMN id BIGINT NOT NULL AUTO_INCREMENT COMMENT '主键ID',
	MODIFY COLUMN captcha_id VARCHAR(32) NOT NULL COMMENT '验证码会话ID（回传前端）',
	...
	MODIFY COLUMN failed_verify_count INT NOT NULL DEFAULT 0 COMMENT '该会话下验证码校验失败次数',
	MODIFY COLUMN created_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) COMMENT '记录创建时间',
	MODIFY COLUMN updated_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) COMMENT '记录更新时间';
