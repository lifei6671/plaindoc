...
import type { AdminSpaceCover, DataGateway, Visibility } from "../../data-access";
...
function buildSystemCoverSVG(spaceName: string): string {
  ...
  const fontSize = maxUnits > 11 ? 148 : maxUnits > 9 ? 164 : 178;
  ...
  const tspans = lines
    .map((line, index) => {
      ...
      return `<tspan x="128" dy="${dy}">${escapeSVGText(line)}</tspan>`;
    })
    .join("");
...
<text x="128" y="${startY}" fill="#192d54" font-size="${fontSize}" font-weight="700" font-family="PingFang SC, Microsoft YaHei, Noto Sans SC, Source Han Sans SC, sans-serif">
${tspans}
</text>
</svg>`;
}
...
export function AdminCreateSpaceDialog({ open, dataGateway, onOpenChange, onCreated }: AdminCreateSpaceDialogProps) {
  ...
  const [coverAsset, setCoverAsset] = useState<AdminSpaceCover | null>(null);
  const [coverSourceLabel, setCoverSourceLabel] = useState<"user_upload" | "system_generated" | null>(null);
  ...
  const coverPreviewUrl = coverAsset?.url?.trim() || "";
  ...
  const handleUploadProcessedCover = useCallback(async () => {
    ...
    try {
      ...
      const payload = await dataGateway.admin.createSpaceCoverAsset({
        source: "user_upload",
        file: exported.file,
        clientWidth: exported.width,
        clientHeight: exported.height,
        clientMimeType: exported.file.type,
        clientProcessed: true
      });
      setCoverAsset(payload);
      setCoverSourceLabel("user_upload");
      showToast("封面上传成功", "success");
    } catch (error) {
      showToast(`封面上传失败：${formatError(error)}`);
    } finally {
      setUploadingCover(false);
    }
  }, [dataGateway.admin, loadedImage, offsetX, offsetY, zoom]);
  ...
  const handleGenerateSystemCover = useCallback(async () => {
    ...
    try {
      ...
      setCoverAsset(payload);
      setCoverSourceLabel("system_generated");
      showToast("系统封面生成并上传成功", "success");
    } catch (error) {
      showToast(`系统封面生成失败：${formatError(error)}`);
    } finally {
      setUploadingCover(false);
    }
  }, [dataGateway.admin, name]);
  ...
  }, [coverAsset?.assetId, dataGateway.admin, description, name, onCreated, onOpenChange, resetDialog, visibility]);
              ...
              <Tabs value={coverMode} onValueChange={(value) => setCoverMode(value as "user_upload" | "system_generated")}>
                ...
                <TabsContent value="user_upload" className="space-y-3">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-xs text-slate-500">目标比例 5:8，前端导出 WebP 后上传</span>
                  </div>
                ...
                <TabsContent value="system_generated" className="space-y-3">
                  <p className="text-xs text-slate-600">系统将基于当前“空间名称”在前端生成简约风 SVG，再转 Canvas 导出 WebP 并上传。</p>
                  <Button
                    type="button"
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-3">
              ...
              {coverAsset ? (
                <div className="mt-2 rounded-md bg-slate-50 p-2 text-[11px] text-slate-600">
                  <p>来源：{(coverSourceLabel ?? coverAsset.source) === "system_generated" ? "系统生成" : "用户上传"}</p>
                  <p>尺寸：{coverAsset.width} × {coverAsset.height}</p>
                  <p>格式：{coverAsset.mimeType}</p>
                </div>
              ) : null}
            </div>
          </aside>
