...
2. `auth_captcha_challenges`
   - 存验证码挑战：`captcha_id`、题目哈希、级别、过期时间、是否已消费。
...
1. 验证码挑战数据：`captchaId`、`captchaImageDataUrl`、`level`、`expiresInSeconds`
...
`/admin/login` 同样对接验证码挑战，避免后台入口在新策略下不可登录。
