...
**目标**: 在文档编辑器右侧子菜单增加“导入”能力，支持 `docx`、`xlsx`、`md`、`txt`、`html`、`zip` 导入；在启用 ONLYOFFICE 时优先保留 Office 原生格式，在未启用时统一转换为 Markdown 文档；支持 ZIP 目录结构恢复、`README.md` 作为目录父文档、空目录抑制、部分失败、失败清单回显、文本编码统一转 UTF-8，以及 Office 内嵌图片落地为 Markdown 图片引用。
...
#### 5.3.3 ZIP / Markdown / HTML 图片路径处理

导入源中的图片引用统一按以下规则解析：

1. 若图片引用是 `http/https` 远程 URL，则在导入时下载图片并转存到图床，再回填受控 URL
2. 若图片引用是以 `/` 开头的绝对路径，则按 ZIP 根目录下的资源路径解析
3. 若图片引用是相对路径，则按当前文件所在目录解析 ZIP 内资源
4. 远程图片下载失败时，允许保留原始远程 URL；ZIP 内已命中的本地图片若上传失败，则该文件导入失败
5. Markdown 图片语法与 HTML `<img>` 标签都要应用同一套路径解析与本地化规则

#### 5.3.4 为什么不使用 data URI
...
1. 目录节点按需创建，不预创建整棵 ZIP 目录树
2. ZIP 会先裁掉所有可导入文件共享的外层壳目录，将首个实际包含可导入文件的目录视为导入根
3. 若 ZIP 根目录或某个子目录下存在 `README.md`、`README.markdown`，或历史兼容别名 `READMD.*`，该文件直接作为该目录层级的父文档，并统一按 `README` 标题导入
4. 某个目录没有自己的 `README` 时，若其祖先目录存在 `README`，则该目录及其后代中的可导入文件默认挂到最近的祖先 `README` 文档下
5. 当某个 `README` 所在目录下存在多个“包含可导入文件的直接子目录”时，`README` 文档与这些子目录在其上级父目录下并列创建，子目录不再挂到该 `README` 文档下
6. 当某个 `README` 所在目录下只有一条可导入子目录链时，该单链目录会被压平，直接处理其后续文件，直到遇到分叉点或下一个 `README`
7. 当子目录自己也存在 `README` 时，从该子目录开始切换为创建这个更近的 `README` 文档；若该子目录下又出现多个直接子目录，则继续按“README 与子目录并列”规则展开
8. README 文档的导入标识默认取其承接的目录名；若当前 `README` 所在目录下存在多个直接子目录，则该 `README` 标识改为随机生成
9. 普通文档的导入标识默认沿用原文件名（保留扩展名，如 `read-1.md`、`read-1.html`）；若发生重复，则自动追加随机后缀
10. 若某个子目录及其后代中不存在任何可导入文件，则该空目录不创建
11. 仅用于 HTML/Markdown 图片引用的资源文件不单独生成文档节点，也不会触发空目录创建
...
- [x] 新增单次导入接口
- [x] 实现工作区编辑权限与目标节点权限校验
- [x] 实现文件类型识别和白名单校验
- [x] 实现文本编码探测与 UTF-8 规范化
- [x] 实现 ZIP 安全解压与路径清洗
- [x] 实现目录结构恢复、README 父文档挂载和重名处理
- [x] 复用现有 Office source blob 链路实现原生 `docx/xlsx` 导入
- [x] 为服务端引入 `turndown` 依赖与转换脚本
- [x] 实现 `docx -> Mammoth -> Turndown -> Markdown`
- [x] 实现 `xlsx -> excelize HTML -> Turndown -> Markdown`
- [x] 实现 `html -> Turndown -> Markdown`
- [x] 实现 Word/Excel 图片物化并替换为 Markdown 图片引用
- [x] 实现复杂图表提示降级
- [x] 实现单次导入结果汇总与失败清单
- [x] 补齐后端单测与路由测试
...
- [x] 实现文件级结果列表
- [x] 实现失败清单展示
- [x] 导入完成后刷新文档树并按需定位新文档
- [x] 补齐 `WorkspaceTree` 相关组件测试

### 13.3 联调与验收

- [x] 验证 ONLYOFFICE 开关分流是否符合预期
- [x] 验证 `docx/xlsx/md/txt/html/zip` 各类型导入
- [x] 验证 UTF-8 与 `GB2312/GBK/GB18030` 文本解码
- [x] 验证 ZIP 混合成功失败场景
- [x] 验证目录结构恢复、README 父文档挂载和空目录抑制
- [x] 验证图片是否正确落成 Markdown 图片引用
- [ ] 验证复杂图表提示是否可见
- [ ] 验证权限拦截与跨空间安全性
