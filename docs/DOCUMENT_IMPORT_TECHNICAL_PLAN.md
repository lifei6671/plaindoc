...
5. 当某个 `README` 所在目录下存在多个“包含可导入文件的直接子目录”时，这些子目录需要作为目录节点保留并挂在该 `README` 文档下
6. 当某个 `README` 所在目录下只有一条可导入子目录链时，该单链目录会被压平，直接处理其后续文件，直到遇到分叉点或下一个 `README`
7. 当子目录自己也存在 `README` 时，从该子目录开始切换为挂到这个更近的 `README` 文档下
8. 若某个子目录及其后代中不存在任何可导入文件，则该空目录不创建
9. 仅用于 HTML/Markdown 图片引用的资源文件不单独生成文档节点，也不会触发空目录创建
