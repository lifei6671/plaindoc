...
5. 文档保存/删除已改为“进程内异步队列”调用（`EnqueueSyncDocumentByID/EnqueueDeleteDocumentByID`），但尚非持久化 Outbox。
6. 权限过滤目前是“索引粗过滤 + DB 二次过滤”混合模式；`min_role` 字段有写入，但查询路径未强制使用 `user_role_level`。
7. 目前还没有 `search_index_jobs` 持久化任务表、重试退避、任务去重合并、断点续跑、索引版本切换。
...
| 编号 | 状态 | 优先级 | 任务 | 主要落地位置 | 验收标准 |
|---|---|---|---|---|---|
| B1 | 待实现 | P0 | 新增 `search_index_jobs`（Outbox）表 | `internal/storage/migrations/*/0024_search_index_jobs.*.sql` | 三库迁移通过，索引与约束齐全 |
| B2 | 待实现 | P0 | 定义任务类型：`DOC_UPSERT/DOC_DELETE/SPACE_PURGE/REBUILD_SPACE/REBUILD_ALL/DICT_VERSION_BUMP` | `models` + `service` 常量 | 枚举可序列化，前后端可读 |
| B3 | 待实现 | P0 | 状态机与重试：`PENDING->RUNNING->SUCCESS/FAILED(RETRY)` + 指数退避+jitter | `search_index_job_service` | 失败任务自动延期重试且可观测 |
| B4 | 待实现 | P0 | 去重合并规则：同 doc upsert 合并，delete 吞并旧 upsert | `repository` 去重 SQL + service | 队列不被高频保存刷爆 |
| B5 | 待实现 | P0 | 优先级策略：权限收紧/删除高于普通 upsert | `priority` 字段 + 拉取 SQL | 高优任务先消费 |
| B6 | 待实现 | P0 | 同事务写入：文档写入与 outbox 入队在一个 DB 事务中提交 | `workspaceRepo/documentRepo/admin service` | 不再出现“文档已变更但任务丢失” |
| B7 | 待实现 | P1 | worker 拉取并发控制：批量拉取、抢占锁、避免多实例重复消费 | `job repo + worker` | 多 worker 下无重复执行 |

### C. 写库侧触发器（事件生成）

| 编号 | 状态 | 优先级 | 任务 | 主要落地位置 | 验收标准 |
|---|---|---|---|---|---|
| C1 | 部分 | P0 | 文档新增/编辑/标题改/可见性改统一写 `DOC_UPSERT` 到 Outbox | `workspace/access handlers -> service/repo` | 所有入口一致走 Outbox |
| C2 | 部分 | P0 | 文档删除写 `DOC_DELETE`（保留当前策略：delete 为主） | `workspace/admin_document` | 删除后不再被旧 upsert 恢复 |
| C3 | 部分 | P1 | 空间删除/清空写 `SPACE_PURGE`，不在请求线程同步清理索引 | `admin_space/workspace/access` | 删除空间接口不被索引耗时阻塞 |
...
| 编号 | 状态 | 优先级 | 任务 | 主要落地位置 | 验收标准 |
|---|---|---|---|---|---|
| D1 | 部分 | P0 | 落地独立 Outbox worker（替代进程内临时队列） | `cmd/server/main.go` + `service` | 进程重启不丢任务 |
| D2 | 部分 | P0 | `DOC_UPSERT` 执行流程标准化：读最新 doc -> 分词/清洗 -> provider.Upsert | `search_index_service` | 与当前全量重建口径一致 |
| D3 | 部分 | P0 | `DOC_DELETE` 标准化：provider.Delete + 状态更新 | `search_index_service` | 删除任务幂等 |
| D4 | 部分 | P1 | `SPACE_PURGE` 批量路径标准化 | `search_index_service` | 大空间删除可控完成 |
...
| 编号 | 状态 | 优先级 | 任务 | 主要落地位置 | 验收标准 |
|---|---|---|---|---|---|
| I1 | 部分 | P0 | 幂等性测试：upsert 合并、delete 吞并旧 upsert | `search_index_service_test` + job tests | 乱序与重复任务不出错 |
...
2. `SyncSpaceByID/PurgeSpaceByID` 当前仍有请求线程同步调用，建议与 C3 同批改为 Outbox。
3. `EnqueueSyncDocumentByID/EnqueueDeleteDocumentByID` 现可作为过渡 API，内部改为“写 Outbox”即可平滑迁移。
